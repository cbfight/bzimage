...
def col_fusion(img_list,rows,cols,overlap):
    ...
    if cols > 1:
        ...
        for row in left_right_rows:
            ...
            for tile in [x for x in range(0,cols)]:
                if i < cols:
                    ...
                    if len(fusion_img.shape) > 2:
                        ...
                        col_margin = int(col_dims * overlap)
                        col_margin_alpha = int(col_margin/1)
                        #print(margin)
                        if i == 0: #left edge tile
                            fusion_img[:,-col_margin_alpha:col_dims,:] = fusion_img[:,-col_margin_alpha:col_dims,:]*0.5 #ALPHA BLENDING CODE ADDITION
                            foo1.append(fusion_img) #ALPHA BLENDING CODE ADDITION 
                            i+=1
                            print('column fusion: L-R left')
                        elif i == cols-1: #right edge tile
                            fusion_img[:,0:col_margin_alpha,:] = fusion_img[:,0:col_margin_alpha,:]*0.5 #ALPHA BLENDING CODE ADDITION
                            foo1.append(fusion_img) #ALPHA BLENDING CODE ADDITION
                            i+=1
                            ...
                            fusion_img[:,0:col_margin_alpha,:] = fusion_img[:,0:col_margin_alpha,:]*0.5 #ALPHA BLENDING CODE ADDITION
                            foo1.append(fusion_img) #ALPHA BLENDING CODE ADDITION
                            middle_col_count += 1 #ALPHA BLENDING CODE ADDITION
                            i+=1
            ...
            if len(fusion_img.shape)>2:
                #print('RGB (fusion_img.shape)>2)')
                
                dst_arry_width = int((fusion_img.shape[1]*len(foo1))-(fusion_img.shape[1]*(middle_col_count+1)*overlap)) #ALPHA BLENDING CODE ADDITION
                #dst_arry_width_p1 = int((fusion_img.shape[1]*len(foo1))-(fusion_img.shape[1]*(middle_col_count+1)*overlap))+1 #ALPHA BLENDING CODE ADDITION
                #dst_arry_width_m1 = int((fusion_img.shape[1]*len(foo1))-(fusion_img.shape[1]*(middle_col_count+1)*overlap))-1 #ALPHA BLENDING CODE ADDITION
                #if dst_arry_width-fusion_img.shape[1]==0:
                #    pass
                #elif dst_arry_width-fusion_img.shape[1]<0: 
                #   dst_arry_width = dst_arry_width_p1 #account for rounding errors1
                #else:
                #    dst_arry_width = dst_arry_width_m1 #account for rounding errors

                dst_arry_col_position = 0 #ALPHA BLENDING CODE ADDITION
                dst_arry = np.full((fusion_img.shape[0],dst_arry_width,3),0) #ALPHA BLENDING CODE ADDITION
                #print('dst_arry.shape %s' % str(dst_arry.shape))
                
                for col in foo1: #ALPHA BLENDING CODE ADDITION
                    #print(dst_arry_col_position+int(col.shape[1]*overlap))
                    dst_arry[:,dst_arry_col_position:dst_arry_col_position+col.shape[1],:] =  dst_arry[:,dst_arry_col_position:dst_arry_col_position+col.shape[1],:]+col[:,:,:] #ALPHA BLENDING CODE ADDITION
                    dst_arry_col_position += int(col.shape[1]-col_margin) #ALPHA BLENDING CODE ADDITION
                    
                column_fusions.append((row,dst_arry))

            else:
                #print('WHITE (fusion_img.shape)<2)')
                ...
                #    dst_arry_width = dst_arry_width_m1 #account for rounding errors
                dst_arry_col_position = 0 #ALPHA BLENDING CODE ADDITION
                dst_arry = np.full((fusion_img.shape[0],dst_arry_width),0) #ALPHA BLENDING CODE ADDITION
                #print('dst_arry.shape %s' % str(dst_arry.shape))
        ...
        for row in right_left_rows:
                ...
                if i < cols:
                    ...
                    if len(fusion_img.shape) > 2:
                        ...
                        if i == 0: #RIGHT edge tile (OPPOSITE FROM ABOVE)
                            fusion_img[:,0:col_margin_alpha,:] = fusion_img[:,0:col_margin_alpha,:]*0.5 #ALPHA BLENDING CODE ADDITION
                            foo2.append(fusion_img) #ALPHA BLENDING CODE ADDITION
                            i+=1 
            ...
            if len(fusion_img.shape)>2:
                ...
                dst_arry_width = int((fusion_img.shape[1]*len(foo2))-(fusion_img.shape[1]*(middle_col_count+1)*overlap)) #ALPHA BLENDING CODE ADDITION
                dst_arry_width_p1 = int((fusion_img.shape[1]*len(foo2))-(fusion_img.shape[1]*(middle_col_count+1)*overlap))+1 #ALPHA BLENDING CODE ADDITION
                dst_arry_width_m1 = int((fusion_img.shape[1]*len(foo2))-(fusion_img.shape[1]*(middle_col_count+1)*overlap))-1 #ALPHA BLENDING CODE ADDITION
                #if dst_arry_width-fusion_img.shape[1]==0:
                #    pass
                #elif dst_arry_width-fusion_img.shape[1]<0: 
                #   dst_arry_width = dst_arry_width_p1 #account for rounding errors1
                ...
                for col in foo2[::-1]: #ALPHA BLENDING CODE ADDITION
                    #print(dst_arry_col_position+int(col.shape[1]*overlap))
                    dst_arry[:,dst_arry_col_position:dst_arry_col_position+col.shape[1],:] =  dst_arry[:,dst_arry_col_position:dst_arry_col_position+col.shape[1],:]+col[:,:,:] #ALPHA BLENDING CODE ADDITION
                    dst_arry_col_position += int(col.shape[1]-col_margin) #ALPHA BLENDING CODE ADDITION
...
def row_fusion(fused_cols,rows,cols,overlap):
    ...
    middle_row_count = 0 #ALPHA BLENDING CODE ADDITION
    if rows > 1:
        j=0
        for cf in [x for x in range(0,rows)]:
            ...
            row_margin = int(row_dims * overlap)
            row_margin_alpha = int(row_margin/1)

            #print(margin)
            if j == 0: #TOP FUSED ROW
                fusion_img[-row_margin_alpha:row_dims,:] = fusion_img[-row_margin_alpha:row_dims,:]*0.5 #ALPHA BLENDING CODE ADDITION
                row_fusions.append(fusion_img) #ALPHA BLENDING CODE ADDITION
                #row_fusions.append(fusion_img[0:-row_margin,:]) #ALPHA BLENDING CODE DELETION
                j+=1
                print('row fusion: top')
            elif j == rows-1: #BOTTOM FUSED ROW
                fusion_img[0:row_margin_alpha,:] = fusion_img[0:row_margin_alpha,:]*0.5 #ALPHA BLENDING CODE ADDITION
                row_fusions.append(fusion_img) #ALPHA BLENDING CODE ADDITION
                #row_fusions.append(fusion_img[row_margin:row_dims,:]) #ALPHA BLENDING CODE DELETION
                j+=1
                ...
                fusion_img[0:row_margin_alpha,:] = fusion_img[0:row_margin_alpha,:]*0.5 #ALPHA BLENDING CODE ADDITION
                row_fusions.append(fusion_img) #ALPHA BLENDING CODE ADDITION
                #row_fusions.append(fusion_img[row_margin:-row_margin,:]) #MIDDLE FUSED ROW(S) #ALPHA BLENDING CODE DELETION
                j+=1
                middle_row_count += 1
        ...
        dst_arry_height = int((fusion_img.shape[0]*len(row_fusions))-(fusion_img.shape[0]*(middle_row_count+1)*overlap)) #ALPHA BLENDING CODE ADDITION
        dst_arry_height_p1 = int((fusion_img.shape[0]*len(row_fusions))-(fusion_img.shape[0]*(middle_row_count+1)*overlap))+1 #ALPHA BLENDING CODE ADDITION
        dst_arry_height_m1 = int((fusion_img.shape[0]*len(row_fusions))-(fusion_img.shape[0]*(middle_row_count+1)*overlap))-1 #ALPHA BLENDING CODE ADDITION
        if dst_arry_height-fusion_img.shape[0] == 0:
            pass
        elif dst_arry_height-fusion_img.shape[1] < 0: 
            dst_arry_height = dst_arry_height_p1 #account for rounding errors1
        else:
            dst_arry_height = dst_arry_height_m1 #account for rounding errors
            
            
        
        print('fusion_img.shape is %s' % str(fusion_img.shape))
        
        if len(fusion_img.shape)>2:
            print('RGB (fusion_img.shape)>2)')
            
            dst_arry_height = int((fusion_img.shape[0]*len(row_fusions))-(fusion_img.shape[0]*(middle_row_count+1)*overlap)) #ALPHA BLENDING CODE ADDITION
            dst_arry_row_position = 0 #ALPHA BLENDING CODE ADDITION
            dst_arry = np.full((dst_arry_height,fusion_img.shape[1],3),0) #ALPHA BLENDING CODE ADDITION
            print('dst_arry.shape %s' % str(dst_arry.shape))
            
            for row in row_fusions: #ALPHA BLENDING CODE ADDITION
                print(dst_arry_row_position+int(row.shape[0]*overlap))
                dst_arry[dst_arry_row_position:dst_arry_row_position+row.shape[0],:,:] =  dst_arry[dst_arry_row_position:dst_arry_row_position+row.shape[0],:,:]+row[:,:,:] #ALPHA BLENDING CODE ADDITION
                dst_arry_row_position += int(row.shape[0]-row_margin) #ALPHA BLENDING CODE ADDITION
            ...
            dst_arry_height = int((fusion_img.shape[0]*len(row_fusions))-(fusion_img.shape[0]*(middle_row_count+1)*overlap)) #ALPHA BLENDING CODE ADDITION
            dst_arry_row_position = 0 #ALPHA BLENDING CODE ADDITION
            dst_arry = np.full((dst_arry_height,fusion_img.shape[1]),0) #ALPHA BLENDING CODE ADDITION
            print('dst_arry.shape %s' % str(dst_arry.shape))
        ...
        final_fusion = dst_arry #ALPHA BLENDING CODE ADDITION
        #final_fusion = cv2.vconcat(row_fusions) #fuse the rows #ALPHA BLENDING CODE DELETION
        return final_fusion
...
def main(tiles,unique_zs,projection_mode='intensity'):
    ...
    if tiles != None:
        ...
        for tile in tiles:
            ...
            tifffile.imwrite(proc_folder+timestamp+'_FF_'+base_name+tile+'_RGB.tif',rgb.astype('uint8')) #can't stitch multicolor images
            ...
            tifffile.imwrite(proc_folder+timestamp+'_FF_'+base_name+tile+'_W.tif',projectImage_w.astype('uint8'))
        ...
        tifffile.imwrite(proc_folder+timestamp+'_FF_'+base_name+'_RGB.tif',rgb.astype('uint8')) #can't stitch multicolor images
        ...
        tifffile.imwrite(proc_folder+timestamp+'_FF_'+base_name+'_W.tif',projectImage_w.astype('uint8'))
...
if __name__ == "__main__":
    ...
    if tiles != None:
        overlap_percent = 0.30 #+0.005+0.0025 #0.3 works but the stitching scars are noticeable #1-17-21, WW: 0.3075 works best
        ...
        tifffile.imwrite(proc_folder+timestamp+'_'+str(lens_info)+'_'+str(row_dims)+'X'+str(col_dims)+'_final_fusion_RGB.tif',fuse_rows_rgb.astype('uint8'))
        ...
        tifffile.imwrite(proc_folder+timestamp+'_'+str(lens_info)+'_'+str(row_dims)+'X'+str(col_dims)+'_final_fusion_W.tif',fuse_rows_w.astype('uint8'))
